...
func TestNginxHelmChart(t *testing.T) {
	...
	options := &helm.Options{
		SetValues: map[string]string{"image.repository": "nginx", "image.tag": "1.15.8"},
	}
	...
	deployContainers := deployment.Spec.Template.Spec.Containers
	if deployContainers[0].Image != expectedContainerImage {
		t.Fatalf("Rendered container image (%s) is not expected (%s)", deployContainers[0].Image, expectedContainerImage)
	}
}
